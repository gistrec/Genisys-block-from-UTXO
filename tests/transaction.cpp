...
// Тестируем вход транзакции
TEST_F(TransactionTest, getSourceBytesInput) {
...
// Тестируем выход транзакции
TEST_F(TransactionTest, getSourceBytesOutput) {
    // Получаем первую транзакцию
    Transaction* transaction = block->getTransactions()[0];

    TransactionOutput* output = transaction->getOutputs()[0];

    vector<byte> bytes = output->getSourceBytes();
    // Не стоило пихать сюда ~100 байтов, но что поделать
    vector<byte> need = {0x00, 0xF2, 0x05, 0x2A, 0x01, 0x00, 0x00, 0x00, 0x43, 0x41, 0x04, 0x67, 0x8A, 0xFD, 0xB0, 0xFE, 0x55, 0x48, 0x27, 0x19, 0x67, 0xF1, 0xA6, 0x71, 0x30, 0xB7, 0x10, 0x5C, 0xD6, 0xA8, 0x28, 0xE0, 0x39, 0x09, 0xA6, 0x79, 0x62, 0xE0, 0xEA, 0x1F, 0x61, 0xDE, 0xB6, 0x49, 0xF6, 0xBC, 0x3F, 0x4C, 0xEF, 0x38, 0xC4, 0xF3, 0x55, 0x04, 0xE5, 0x1E, 0xC1, 0x12, 0xDE, 0x5C, 0x38, 0x4D, 0xF7, 0xBA, 0x0B, 0x8D, 0x57, 0x8A, 0x4C, 0x70, 0x2B, 0x6B, 0xF1, 0x1D, 0x5F, 0xAC};
    ASSERT_EQ(bytes, need);
}